...
// dep loaders
const styleLoaderPath = normalize.dep('viola-style-loader'/* 'vue-style-loader' */)
...
function resolveLoaders (
  ...
  function getCSSExtractLoader (lang) {
    ...
    return extractor.extract({
      ...
      fallback: 'viola-style-loader' /* 'vue-style-loader' */
    })
  }
...
module.exports = function createHelpers (
  ...
  function getRequestString (type, part, index, scoped) {
    ...
    var loaderString = ''
    let res = loaderUtils.stringifyRequest(
      loaderContext,
      // disable all configuration loaders
      '!!' +
      // get loader string for pre-processors
      (loaderString = getLoaderString(type, part, index, scoped)) +
      // select the corresponding part from the vue file
      getSelectorString(type, index || 0) +
      // the url to the actual vue file, including remaining requests
      rawRequest
    )
    // type == 'styles' && _c.log(['stringifyRequest', type], { res, rawRequest, loaderString })
    return res
  }
  ...
  function addCssModulesToLoader (loader, part, index) {
    // _c.log('addCssModulesToLoader loader', loader)
  ...
  function getLoaderString (type, part, index, scoped) {
    ...
    // type == 'styles' && _c.log(['helper getLoaderString'], loader)
  ...
  function getRawLoaderString (type, part, index, scoped) {
    ...
    if (type === 'styles') {
      ...
      styleCompiler =
        ...
        '!'
      // _c.log('getRawLoaderString styleCompiler', styleCompiler)
    ...
    if (loader != null) {
      ...
      if (type === 'styles') {
        // add css modules
        // _c.log('before addCssModulesToLoader', loader)
